...
class CaptchaVerify
{
	...
	public function verify_code()
	{
		...
		if (isset($_POST['submit_form']) && !empty($this->captcha_value) && !empty($this->captcha_field) && !empty($this->answer_time))
		{
			...
			if($this->current_time - $this->answer_time < 6) $this->error_msg('Вы или робот или вводите капчу слишком быстро!');
			if($_POST[$this->captcha_field] == '') $this->error_msg('Робот, уходи!');
			
			if(md5(md5($_POST[$this->captcha_field])) == $this->captcha_value) echo 'Капча пройдена успешно!';
			else $this->error_msg('Неверная капча!');
		}
